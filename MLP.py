...
OUTPUT_DIR = 'mlp_multi_output_model'
os.makedirs(OUTPUT_DIR, exist_ok=True)
try:
    with open('hyperparameter_tuning_results/mlp_best_params.json', 'r') as f:
        tuned_params = json.load(f)
    print("Loaded best MLP hyperparameters.")
except FileNotFoundError:
    print("Error: mlp_best_params.json not found. Run the hyperparameter tuning script first.")
    exit()
...
def run_mlp_experiment(seed: int, df: pd.DataFrame, best_params: dict):
    print(f"\n--- Running MLP Experiment with Seed: {seed} ---")
    ...
    # Training set
    train_preds_scaled = best_model.predict(X_train_scaled)
    train_pred = scaler_y.inverse_transform(train_preds_scaled)
    ...
    # Full dataset
    full_preds_scaled = best_model.predict(scaler_X.transform(X))
    full_pred = scaler_y.inverse_transform(full_preds_scaled)
...
print("\n--- Starting 50-Run MLP Evaluation ---")
...
for i in range(num_runs):
    test_scores, train_scores, full_scores, train_report, test_report, full_report = run_mlp_experiment(seed=i, df=df_main, best_params=tuned_params)
...
print(f"\n--- Final Aggregated MLP Results from {num_runs} Runs ---")

print("\nMetrics for TEST SET (Primary Result):")
...
print("\nMetrics for TRAINING SET (Diagnostics):")
for feature, scores in all_train_r2.items():
    mean_r2 = np.mean(scores)
    print(f"  {feature}: Average R-squared = {mean_r2:.4f}")

print("\nMetrics for FULL DATASET (Diagnostics):")
...
testing_csv_path = os.path.join(OUTPUT_DIR, 'mlp_testing_predictions_all_runs.csv')
...
training_csv_path = os.path.join(OUTPUT_DIR, 'mlp_training_predictions_all_runs.csv')
...
full_csv_path = os.path.join(OUTPUT_DIR, 'mlp_full_dataset_predictions_all_runs.csv')
...
raw_scores_filename = os.path.join(output_dir_2, 'raw_r2_scores_mlp.json')
...
print("\nMLP — TEST 20% R2 across seeds (mean +/- std, 95% CI)")
...
r2_entry_test.to_csv('r2_analysis_results/mlp_test_r2_per_seed_entry.csv', index=False)
r2_exit_test.to_csv('r2_analysis_results/mlp_test_r2_per_seed_exit.csv', index=False)
...
print("\nMLP — FULL 112 R2 across seeds (mean +/- std, 95% CI)")
...
r2_entry_full_seeds.to_csv('r2_analysis_results/mlp_full_r2_per_seed_entry.csv', index=False)
r2_exit_full_seeds.to_csv('r2_analysis_results/mlp_full_r2_per_seed_exit.csv', index=False)
