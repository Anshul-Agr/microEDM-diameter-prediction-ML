...
OUTPUT_DIR = 'svm_multi_output_model'
os.makedirs(OUTPUT_DIR, exist_ok=True)

try:
    with open('hyperparameter_tuning_results/svm_best_params.json', 'r') as f:
        tuned_params = json.load(f)
    best_params = {key.replace('estimator__', ''): value for key, value in tuned_params.items()}
    print("Loaded best SVM hyperparameters.")
    print(f"Using parameters: {best_params}")
except FileNotFoundError:
    print("Error: svm_best_params.json not found. Run the hyperparameter tuning script first.")
    exit()
...
def run_svm_experiment(seed: int, df: pd.DataFrame, model_params: dict):
    ...
    # --- B. Model Training ---
    svm_base = SVR(**model_params)
    best_model = MultiOutputRegressor(estimator=svm_base, n_jobs=1)
...
print("\n--- Starting 50-Run SVM Evaluation ---")
...
for i in range(num_runs):
    test_scores, train_scores, full_scores, train_report, test_report, full_report = run_svm_experiment(seed=i, df=df_main, model_params=best_params)
...
print(f"\n--- Final Aggregated SVM Results from {num_runs} Runs ---")
print("\nTest SET:")
...
testing_csv_path = os.path.join(OUTPUT_DIR, 'svm_testing_predictions_all_runs.csv')
...
training_csv_path = os.path.join(OUTPUT_DIR, 'svm_training_predictions_all_runs.csv')
...
full_csv_path = os.path.join(OUTPUT_DIR, 'svm_full_dataset_predictions_all_runs.csv')
...
raw_scores_filename = os.path.join(output_dir_2, 'raw_r2_scores_svm.json')
...
print("\nSVM -- TEST 20% R2 across seeds (mean +/- std, 95% CI)")
...
r2_entry_test.to_csv('r2_analysis_results/svm_test_r2_per_seed_entry.csv', index=False)
r2_exit_test.to_csv('r2_analysis_results/svm_test_r2_per_seed_exit.csv', index=False)
...
print("\nSVM -- FULL 112 R2 across seeds (mean +/- std, 95% CI)")
...
r2_entry_full_seeds.to_csv('r2_analysis_results/svm_full_r2_per_seed_entry.csv', index=False)
r2_exit_full_seeds.to_csv('r2_analysis_results/svm_full_r2_per_seed_exit.csv', index=False)
