...
OUTPUT_DIR = 'lgbm_multi_output_model'
...
try:
    with open('hyperparameter_tuning_results/lgbm_best_params.json', 'r') as f:
        tuned_params = json.load(f)
    print("Loaded best LightGBM hyperparameters.")
except FileNotFoundError:
    print("Error: lgbm_best_params.json not found. Run the hyperparameter tuning script first.")
    exit()

# --- 1. Main Experiment Function ---
def run_experiment(seed: int, df: pd.DataFrame, all_best_params: dict):
    print(f"\n--- Running LightGBM Experiment with Seed: {seed} ---")
    ...
    for feature in output_features:
        ...
        feature_params['verbose'] = -1

        model = lgb.LGBMRegressor(**feature_params)
    ...
    model_path = os.path.join(OUTPUT_DIR, f"lgb_models_seed{seed}.joblib")
    joblib.dump(best_model_collection, model_path)
...
print("\n--- Starting 50-Run LightGBM Evaluation ---")
start_eval_time = time.time()
num_runs = 50
all_test_r2 = {feature: [] for feature in output_features}
all_train_r2 = {feature: [] for feature in output_features}
all_full_r2 = {feature: [] for feature in output_features}
all_train_reports, all_full_reports, all_test_reports = [], [], []

for i in range(num_runs):
    test_scores, train_scores, full_scores, train_report, test_report, full_report = run_experiment(seed=i, df=df_main, all_best_params=tuned_params)
    for feature in output_features:
...
print(f"\n--- Final Aggregated LightGBM Results from {num_runs} Runs ---")
print("\nTest SET:")
...
testing_csv_path = os.path.join(OUTPUT_DIR, 'lgbm_testing_predictions_all_runs.csv')
...
training_csv_path = os.path.join(OUTPUT_DIR, 'lgbm_training_predictions_all_runs.csv')
...
full_csv_path = os.path.join(OUTPUT_DIR, 'lgbm_full_dataset_predictions_all_runs.csv')
...
raw_scores_filename = os.path.join(output_dir_2, 'raw_r2_scores_lightgbm.json')
...
print("\nLightGBM -- TEST 20% R2 across seeds (mean +/- std, 95% CI)")
...
r2_entry_test.to_csv('r2_analysis_results/test_r2_per_seed_entry.csv', index=False)
r2_exit_test.to_csv('r2_analysis_results/test_r2_per_seed_exit.csv', index=False)
...
print("\nLightGBM -- FULL 112 R2 across seeds (mean +/- std, 95% CI)")
...
r2_entry_full_seeds.to_csv('r2_analysis_results/full_r2_per_seed_entry.csv', index=False)
r2_exit_full_seeds.to_csv('r2_analysis_results/full_r2_per_seed_exit.csv', index=False)
